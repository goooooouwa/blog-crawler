...
See examples of RSS feeds (`feeds.txt`, `rss-[0-9].xml`) & podcast URLs (`mp3-urls.txt`) generated by Blog Crawler [here](https://github.com/goooooouwa/rss-feeds).
...
## How to crawl websites and generate RSS feeds with Blog Crawler
...
```bash
ruby ./bin/run.rb render coding_horror   # generate RSS feeds `rss-[0-9].xml` and a `feeds.txt` with their URLs under config["our_dir"] folder (each RSS feed has a set number of posts)
```
