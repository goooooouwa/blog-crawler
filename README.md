...
See examples of RSS feeds (`feeds.txt`, `rss-[0-9].xml`) & podcast URLs (`mp3-urls.txt`) generated by blog_crawler [here](https://github.com/goooooouwa/rss-feeds).
...
```bash
ruby ./bin/run.rb render coding_horror   # generate and save RSS feeds `rss-[0-9].xml` in config["our_dir"] (each RSS feed has a set number of posts)
```
